...
static void print_main_arena(RCore *core, ut64 m_arena, RHeap_MallocState *main_arena, int format) {
	int i, j, k, offset, start;
	ut64 apart[NSMALLBINS + 1] = {0LL};

	if (format == '*') {
		for (i = 0; i < NBINS * 2 - 2; i += 2) {
			offset = (ut64)(size_t)&main_arena->last_remainder - (ut64)(size_t)&main_arena->mutex + SZ;
			ut64 addr = (ut64)(size_t)m_arena + (ut64)(size_t)offset + SZ * i - SZ * 2;
	...
	for (i = 0, j = 1, k = SZ * 4; i < NBINS * 2 - 2; i += 2, j++) {
		...
		ut64 bin = (ut64)(size_t)m_arena + (ut64)(size_t)offset + SZ * i - SZ * 2;
		PRINTF_GA ("0x%"PFMT64x"->fd = ", bin);
		ut64 bin_content = (ut64)(size_t)main_arena->bins[i];
		PRINTF_BA ("0x%"PFMT64x, bin_content);
		PRINT_GA (", ");
		PRINTF_GA ("0x%"PFMT64x"->bk = ", bin);
		bin_content = (ut64)(size_t)main_arena->bins[i + 1];
		PRINTF_BA ("0x%"PFMT64x, bin_content);
		PRINT_GA (", ");
		r_cons_print ("\n");
	}
...
static int str_start_with(const char *ptr, const char *str) {
	return !strncmp (ptr, str, (ut64)(size_t)strlen (str));
}
		...
		if (r_file_exists (path)) {
			...
			if (libc_addr != UT64_MAX && vaddr && vaddr != UT64_MAX) {
				*m_arena = libc_addr + vaddr;
				...
				eprintf ("Warning: Symbol main_arena could not be found. Is libc6-dbg (or libc6-dbg:i386) installed?\n");
				return true;
			}
		} else {
			eprintf ("Warning: glibc library with symbol main_arena could not be found. Is libc6-dbg (or libc6-dbg:i386) installed?\n");
			return true;
		}
...
static int print_double_linked_list_bin_simple(RCore *core, ut64 bin, RHeap_MallocState *main_arena, ut64 brk_start) {
	ut64 next = UT64_MAX;
	RHeapChunk *cnk = R_NEW0 (RHeapChunk);
	if (!cnk) {
		eprintf ("Warning: out of memory\n");
...
static int print_double_linked_list_bin_graph(RCore *core, ut64 bin, RHeap_MallocState *main_arena, ut64 brk_start) {
        RAGraph *g = r_agraph_new (r_cons_canvas_new (1, 1));
	g->can->color = r_config_get_i (core->config, "scr.color");

	ut64 next = UT64_MAX;
	char title[256] = {0}, chunk[256] = {0};
...
static int print_double_linked_list_bin(RCore *core,  RHeap_MallocState *main_arena, ut64 m_arena, ut64 offset, ut64 num_bin, int graph) {
	int ret = 0;
	RListIter *iter;
	RDebugMap *map;
	ut64 brk_start = UT64_MAX;
	ut64 bin = (ut64)(size_t)main_arena->bins[num_bin];
	if (!bin) {
		return ret;
	}
	...
	bin = (ut64)(size_t)m_arena + (ut64)(size_t)offset + SZ * num_bin * 2 - SZ * 2;
	...
	if (!graph || graph == 1) {
		ret = print_double_linked_list_bin_simple (core, bin, main_arena, brk_start);
	} else {
		ret = print_double_linked_list_bin_graph (core, bin, main_arena, brk_start);	
	}
...
static void print_heap_bin(RCore *core, RHeap_MallocState *main_arena, ut64 m_arena, const char *input) {
	...
	ut64 offset = (ut64)(size_t)&main_arena->last_remainder - (ut64)(size_t)&main_arena->mutex + SZ;
	...
	case '\0': // dmhb
		...
		for (i = 0; i < NBINS - 1; i++) {
			PRINTF_YA (" Bin %03d:\n", i + 1);
			if (!print_double_linked_list_bin (core, main_arena, m_arena, offset, i, 0)) {
				PRINT_GA ("  Empty bin");
				PRINT_BA ("  0x0\n");
			} 
		}
	...
	case 'g': // dmhbg [bin_num]
		...
		PRINTF_YA ("  Bin %03d:\n", num_bin + 1);
		if (!print_double_linked_list_bin (core, main_arena, m_arena, offset, num_bin, j)) {
			PRINT_GA ("Empty bin");
			PRINT_BA (" 0x0\n");
		}
		break;
...
static int print_single_linked_list_bin(RCore *core, RHeap_MallocState *main_arena, ut64 m_arena, ut64 offset, ut64 bin_num) {
	ut64 bin = (ut64)(size_t)main_arena->fastbinsY[bin_num];
	if (!bin) {
		return 0;
	}
	...
	bin = (ut64)(size_t)m_arena + (ut64)(size_t)offset + SZ * bin_num;
	if (!bin) {
...
static void print_heap_fastbin(RCore *core, RHeap_MallocState *main_arena, ut64 m_arena, const char *input) {
	int i;
	ut64 num_bin = UT64_MAX;
	ut64 offset = (ut64)(size_t)&main_arena->fastbinsY - (ut64)(size_t)&main_arena->mutex;
...
static void print_heap_graph(RCore *core, RHeap_MallocState *main_arena, ut64 *initial_brk) {
	int w, h;
	...
	RANode *top = {0}, *chunk_node = {0}, *prev_node = {0};
        RListIter *iter;
        RDebugMap *map;
        RHeapChunk *cnk = R_NEW0 (RHeapChunk), *prev_c = R_NEW0 (RHeapChunk);

        ut64 next_chunk_ref, prev_chunk_ref, brk_start, brk_end, size_tmp;
        ...
        r_list_foreach (core->dbg->maps, iter, map) {
                if (strstr (map->name, "heap")) {
                        *initial_brk = (map->addr >> 12) << 12;
			brk_start = map->addr;
                        top_data = r_str_newf ("[brk_start:0x%"PFMT64x", brk_end:0x%"PFMT64x"]\n", (ut64)(size_t)brk_start, (ut64)(size_t)map->addr_end);
        ...
        if (*initial_brk == UT64_MAX) {
                eprintf ("No Heap section\n");
                return;
        }

	top_size = (ut64)(size_t)main_arena->top - brk_start;
        while (next_chunk_ref != (ut64)(size_t)main_arena->top && next_chunk_ref != brk_end) {
                r_core_read_at (core, next_chunk_ref, (ut8 *)prev_c, sizeof (RHeapChunk));
               	node_title = r_str_newf ("  Malloc chunk @ 0x%"PFMT64x" ", (ut64)(size_t)(prev_chunk_ref));
		size_tmp = (prev_c->size >> 3) << 3;
		if (top_size != UT64_MAX && (size_tmp > top_size  || next_chunk_ref + size_tmp > (ut64)(size_t)main_arena->top)) {
			node_data = r_str_newf ("[corrupted] size: 0x%x\n fd:%"PFMT64x", bk:%"PFMT64x"\nHeap graph could not be recovered\n", prev_c->size, prev_c->fd, prev_c->bk) ;
			...
			r_core_read_at (core, next_chunk_ref, (ut8 *)cnk, sizeof (RHeapChunk));
			node_data = r_str_newf ("size: 0x%x\n fd:%"PFMT64x", bk:%"PFMT64x"\n", prev_c->size, prev_c->fd, prev_c->bk) ;
			
		}
			chunk_node = r_agraph_add_node (g, node_title, node_data);
			(first_node) ? first_node = false : r_agraph_add_edge (g, prev_node, chunk_node);
...
static void print_current_heap(RCore *core, RHeap_MallocState *main_arena, ut64 *initial_brk) {
	RListIter *iter;
	RDebugMap *map;
	ut64 brk_start, brk_end, size_tmp;
	ut64 top_size = UT64_MAX;
	RHeapChunk *cnk = R_NEW0 (RHeapChunk);
	...
	top_size = (ut64)(size_t)main_arena->top - brk_start;
	while (next_chunk != (ut64)(size_t)main_arena->top && next_chunk != brk_end) {
		r_core_read_at (core, next_chunk, (ut8 *)cnk, sizeof (RHeapChunk));
		size_tmp = (cnk->size >> 3) << 3;
		if (top_size != UT64_MAX && (size_tmp > top_size || next_chunk + size_tmp > (ut64)(size_t)main_arena->top)) {
			PRINT_YA ("\n  Malloc chunk @ ");
			PRINTF_BA ("0x%"PFMT64x" ", next_chunk);
			PRINT_RA ("[corrupted]\n");
			PRINTF_RA ("   size:%"PFMT64x"\n   fd:%"PFMT64x", bk:%"PFMT64x"\n", cnk->size, cnk->fd, cnk->bk);
			break;
		} else {
			PRINT_YA ("  Malloc chunk @ ");
			PRINTF_BA ("0x%"PFMT64x" ", (ut64)(size_t)(prev_chunk));
			next_chunk += size_tmp;
			prev_chunk = next_chunk;
			r_core_read_at (core, next_chunk, (ut8 *)cnk, sizeof (RHeapChunk));
			(cnk->size % 2 == 0) ? PRINT_GA ("[free]\n") : (next_chunk == brk_end) ? PRINT_GA ("[free]\n") : PRINT_GA ("[allocated]\n");
		}
	}
	PRINT_YA ("\n  Top chunk @ ");
	PRINTF_BA ("0x%"PFMT64x, (ut64)(size_t)(main_arena->top));
	PRINT_GA (" - [brk_start: ");
	PRINTF_BA ("0x%"PFMT64x, (ut64)(size_t)brk_start);
	PRINT_GA (", brk_end: ");
	PRINTF_BA ("0x%"PFMT64x, (ut64)(size_t)brk_end);
	PRINT_GA ("]\n");
	r_cons_printf ("\n");
	free (cnk);
}
...
static int cmd_debug_map_heap(RCore *core, const char *input) {
	...
	case '\0': // "dmh"
		if (!r_resolve_main_arena (core, &m_arena, main_arena)) {
			print_current_heap (core, main_arena, &initial_brk);
		}
		break;
	case '*':
	case 'a': // "dmha"
		if (!r_resolve_main_arena (core, &m_arena, main_arena)) {
			print_main_arena (core, m_arena, main_arena, *input);
		}
		break;
	case 'b': // "dmhb"
		if (!r_resolve_main_arena (core, &m_arena, main_arena)) {
	...
	case 'g': // "dmhg"
		if (!r_resolve_main_arena (core, &m_arena, main_arena)) {
			print_heap_graph (core, main_arena, &initial_brk);
		}
		break;
	case 'j': // "dmhj"
